...
echo =====4-0=====
OLD_DEV=$(aws ec2 describe-instances --filters "Name=tag:Name,Values=gj2026-keycloak-dev-ec2" "Name=instance-state-name,Values=pending,running,stopping,stopped" --region eu-central-1 --query 'Reservations[].Instances[].InstanceId' --output text); OLD_SEC=$(aws ec2 describe-instances --filters "Name=tag:Name,Values=gj2026-keycloak-sec-ec2" "Name=instance-state-name,Values=pending,running,stopping,stopped" --region eu-central-1 --query 'Reservations[].Instances[].InstanceId' --output text); [ -n "$OLD_DEV" ] && aws ec2 terminate-instances --instance-ids $OLD_DEV --region eu-central-1 > /dev/null; [ -n "$OLD_SEC" ] && aws ec2 terminate-instances --instance-ids $OLD_SEC --region eu-central-1 > /dev/null; [ -n "$OLD_DEV" ] && aws ec2 wait instance-terminated --instance-ids $OLD_DEV --region eu-central-1; [ -n "$OLD_SEC" ] && aws ec2 wait instance-terminated --instance-ids $OLD_SEC --region eu-central-1; AMI=$(aws ec2 describe-images --owners amazon --filters "Name=name,Values=al2023-ami-2023*-x86_64" "Name=state,Values=available" --region eu-central-1 --query 'sort_by(Images, &CreationDate)[-1].ImageId' --output text); DEV=$(aws ec2 run-instances --image-id $AMI --instance-type t3.small --region eu-central-1 --tag-specifications 'ResourceType=instance,Tags=[{Key=team,Value=dev-team},{Key=Name,Value=gj2026-keycloak-dev-ec2}]' --query 'Instances[0].InstanceId' --output text); SEC=$(aws ec2 run-instances --image-id $AMI --instance-type t3.small --region eu-central-1 --tag-specifications 'ResourceType=instance,Tags=[{Key=team,Value=sec-team},{Key=Name,Value=gj2026-keycloak-sec-ec2}]' --query 'Instances[0].InstanceId' --output text); aws ec2 wait instance-running --instance-ids $DEV $SEC --region eu-central-1; aws ec2 stop-instances --instance-ids $DEV --region eu-central-1 > /dev/null; aws ec2 wait instance-stopped --instance-ids $DEV --region eu-central-1
echo
...
echo =====4-4=====
aws ec2 start-instances --instance-ids $DEV --region eu-central-1 --profile gj2026-keycloak-dev | jq -r '.StartingInstances[0] | "gj2026-keycloak-dev-ec2: \(.PreviousState.Name) → \(.CurrentState.Name)"'; aws ec2 stop-instances --instance-ids $SEC --region eu-central-1 --profile gj2026-keycloak-sec | jq -r '.StoppingInstances[0] | "gj2026-keycloak-sec-ec2: \(.PreviousState.Name) → \(.CurrentState.Name)"'
echo

echo =====4-5=====
aws ec2 stop-instances --instance-ids $SEC --region eu-central-1 --profile gj2026-keycloak-dev 2>&1 | grep -o "UnauthorizedOperation"; aws ec2 start-instances --instance-ids $DEV --region eu-central-1 --profile gj2026-keycloak-sec 2>&1 | grep -o "UnauthorizedOperation"
echo
...
ADMIN_TOKEN=$(curl -sk -X POST \
  ...
  -d "client_id=admin-cli" -d "username=admin" -d "password=$ADMIN_PASSWORD" \
  -d "grant_type=password" | jq -r '.access_token')
EXISTING_USER_ID=$(curl -sk -H "Authorization: Bearer $ADMIN_TOKEN" \
  "https://$KEYCLOAK_IP/admin/realms/team/users?username=dev-user2&exact=true" | jq -r '.[0].id // empty')
[ -n "$EXISTING_USER_ID" ] && curl -sk -X DELETE \
  -H "Authorization: Bearer $ADMIN_TOKEN" \
  "https://$KEYCLOAK_IP/admin/realms/team/users/$EXISTING_USER_ID"
curl -sk -X POST \
  -H "Authorization: Bearer $ADMIN_TOKEN" \
...
DEV_INSTANCE_ID=$(aws ec2 describe-instances \
  --filters "Name=tag:Name,Values=gj2026-keycloak-dev-ec2" "Name=instance-state-name,Values=running" \
  --region eu-central-1 \
...
aws configure set credential_process \
  "$HOME/.aws/gj2026-keycloak-creds.sh dev dev-user2" \
